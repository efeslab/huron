...
static void *big_alloc_aux(size_t size)
{
	...
	/* Get memory */
#ifndef WINDOWS
	void *p = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS|MAP_HUGETLB|MAP_HUGE_2MB, -1, 0);
#else
...
static void *aligned_alloc_aux2(size_t align, size_t size, size_t rsize)
{
	...
	ps = mmap(NULL, psize, PROT_READ | PROT_WRITE, flags, -1, 0); // TODO: Need HUGETLB here?
