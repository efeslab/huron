...
extern void* adjust_TMI_slab_memory(size_t size);
extern void* get_TMI_mmap_memory(size_t size);
extern int return_TMI_mmap_memory(void *ptr, size_t size);
...
static void init_sbrk(void)
{
	...
	{
#ifndef WINDOWS
	  sbrk_mmap_base = get_TMI_mmap_memory(size);
...
static void *big_alloc_aux(size_t size)
{
	...
	/* Get memory */
#ifndef WINDOWS
	void *p = get_TMI_mmap_memory(size);
...
static inline void big_free_aux(size_t *p)
{
  ...
  return_TMI_mmap_memory(p, *p); //munmap(p, *p);
}
#endif
...
#else
	return_TMI_mmap_memory(page_start(tl2), PAGESIZE); //munmap(page_start(tl2), PAGESIZE);
#endif
...
#else
		return_TMI_mmap_memory(page_start(tl),PAGESIZE); //munmap(page_start(tl), PAGESIZE);
#endif
		return NULL;
	}
...
static atls *init_tls(void)
{
	...
	tl = get_TMI_mmap_memory(PAGESIZE);
...
static freesb *slab_alloc_chunk(atls *tl)
{
	...
	fsb = adjust_TMI_slab_memory(alloc_amount); //sbrk(alloc_amount);
...
#ifndef WINDOWS
	return_TMI_mmap_memory(m,msize);//munmap(m, msize);
#else
...
static noinline void *realloc_aux2(void *p, size_t size, atls *tl)
{
	...
	if (!msize)
	{
		...
		if (size >= BTMALLOC)
		{
			...
			{
#ifndef WINDOWS
				if (!return_TMI_mmap_memory(shift(ps, size), old_size - size))
				{
					/* Update size */
					*ps = size;
				}
...
static void unmap_range(void *x1, void *x2)
{
	if (x1 != x2)
	{
		return_TMI_mmap_memory(x1, (char *) x2 - (char *) x1);
	}
}
#endif
...
static void *aligned_alloc_aux2(size_t align, size_t size, size_t rsize)
{
	...
	ps = get_TMI_mmap_memory(psize);
	  //mmap(NULL, psize, PROT_READ | PROT_WRITE, flags, -1, 0);
