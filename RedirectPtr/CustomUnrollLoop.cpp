...
static inline void remapInstruction(Instruction *I, ValueToValueMapTy &VMap) {
    for (unsigned op = 0, E = I->getNumOperands(); op != E; ++op) {
        ...
        auto wrap = [&](Value *V) {
            auto &C = I->getContext();
            return Wrapped ? MetadataAsValue::get(C, ValueAsMetadata::get(V))
                           : V;
        };
...
CustomLoopUnrollResult CustomUnrollLoop(
    ...
    std::unordered_map<Instruction *, std::vector<Instruction *>> &instHooks) {

    BasicBlock *Preheader = L->getLoopPreheader();
    if (!Preheader) {
        LLVM_DEBUG(
            dbgs() << "  Can't unroll; loop preheader-insertion failed.\n");
        return CustomLoopUnrollResult::Unmodified;
    }

    BasicBlock *LatchBlock = L->getLoopLatch();
    if (!LatchBlock) {
        LLVM_DEBUG(
            dbgs() << "  Can't unroll; loop exit-block-insertion failed.\n");
        return CustomLoopUnrollResult::Unmodified;
    }
    ...
    if (!BI || BI->isUnconditional()) {
        ...
        LLVM_DEBUG(dbgs() << "  Can't unroll; loop not terminated by a "
                             "conditional branch.\n");
        return CustomLoopUnrollResult::Unmodified;
    }
    ...
    if (!CheckSuccessors(0, 1) && !CheckSuccessors(1, 0)) {
        LLVM_DEBUG(
            dbgs() << "Can't unroll; only loops with one conditional latch"
    ...
    // just conservatively assume that they're inserted to preserve LCSSA form,
    // which means that complete unrolling might break this form. We need to
    // either fix it in-place after the transformation, or entirely rebuild
    // LCSSA. TODO: For now we just recompute LCSSA for the outer loop, but it
    // should be possible to fix it in-place.
    ...
    if (RuntimeTripCount && TripMultiple % Count != 0 &&
        !UnrollRuntimeLoopRemainder(L, Count, AllowExpensiveTripCount,
                                    EpilogProfitability, UnrollRemainder, LI,
                                    SE, DT, AC, PreserveLCSSA)) {
        if (Force)
            RuntimeTripCount = false;
        else {
            LLVM_DEBUG(dbgs()
                       << "Wont unroll; remainder loop could not be generated"
    ...
    if (CompletelyUnroll) {
        ...
        if (ORE)
            ORE->emit([&]() {
                return OptimizationRemark(DEBUG_TYPE, "FullyUnrolled",
        ...
        auto DiagBuilder = [&]() {
            OptimizationRemark Diag(DEBUG_TYPE, "PartialUnrolled",
                                    L->getStartLoc(), L->getHeader());
        ...
        if (TripMultiple == 0 || BreakoutTrip != TripMultiple) {
            ...
            LLVM_DEBUG(dbgs()
                       << " with " << TripMultiple << " trips per branch");
            if (ORE)
                ORE->emit([&]() {
                    return DiagBuilder()
    ...
    // We are going to make changes to this loop. SCEV may be keeping cached
    // info about it, in particular about backedge taken count. The changes we
    // make are guaranteed to invalidate this information for our loop. It is
    // tempting to only invalidate the loop being unrolled, but it is incorrect
    // as long as all exiting branches from all inner loops have impact on the
    // outer loops, and if something changes inside them then any of outer loops
    // may also change. When we forget outermost loop, we also forget all
    // contained loops and this is what we need here.
    if (SE)
    ...
    // for PHI nodes.  Insert associations now.
    ...
    // @ours
    // Get a BasicBlock -> Instruction table so we don't have to check for all
    // Instructions when we clone a BasicBlock.
    std::unordered_map<BasicBlock *, std::vector<Instruction *>> bbCheckList;
    for (const auto &p : instHooks)
        bbCheckList[p.first->getParent()].push_back(p.first);
    ...
    for (unsigned It = 1; It != Count; ++It) {
        ...
        for (LoopBlocksDFS::RPOIterator BB = BlockBegin; BB != BlockEnd; ++BB) {
            ...
            if (*BB == Header)
                // Loop over all of the PHI nodes in the block, changing them to
                // use the incoming values from the previous block.
            ...
            // relations in the copied body are the same as in the original
            // body.
            if (DT) {
                if (*BB == Header)
                    ...
                    DT->addNewBlock(
                        New, cast<BasicBlock>(
                                 LastValueMap[cast<Value>(OriginalBBIDom)]));
                }
            }

            // @ours
            // If a BB is on the list, after all mods to this BB is done, 
            // find all the hooked instructions in the cloned new BB.
            auto it = bbCheckList.find(*BB);
            if (it != bbCheckList.end()) {
                const auto &originals = it->second;
                for (auto *inst: originals) {
                    auto it = VMap.find(inst);
                    assert(it != VMap.end() && it->second.pointsToAliveValue());
                    Instruction *cloned = cast<Instruction>(it->second);
                    assert(cloned != nullptr);
                    instHooks[inst].push_back(cloned);
                }
            }
        }
    ...
    for (PHINode *PN : OrigPHINode) {
        if (CompletelyUnroll) {
            ...
            assert(Latches.back() == LastValueMap[LatchBlock] &&
                   "bad last latch");
    ...
    for (unsigned i = 0, e = Latches.size(); i != e; ++i) {
        ...
        if (CompletelyUnroll) {
            ...
            // If we know the trip count or a multiple of it, we can safely use
            // an unconditional branch for some iterations.
            NeedConditional = false;
        }
    ...
    if (DT && Count > 1) {
        for (auto *BB : OriginalLoopBlocks) {
            ...
            BasicBlock *NewIDom;
            if (BB == LatchBlock) {
                // The latch is special because we emit unconditional branches
                // in some cases where the original loop contained a conditional
                // branch. Since the latch is always at the bottom of the loop,
                // if the latch dominated an exit before unrolling, the new
                // dominator of that exit must also be a latch.  Specifically,
                // the dominator is the first latch which ends in a conditional
                // branch, or the last latch if there is no such latch.
                ...
                // The new idom of the block will be the nearest common
                // dominator of all copies of the previous idom. This is
                // equivalent to the nearest common dominator of the previous
                // idom and the first latch, which dominates all copies of the
                // previous idom.
    ...
    for (BasicBlock *Latch : Latches) {
        ...
        if (Term->isUnconditional()) {
            ...
            if (BasicBlock *Fold = foldBlockIntoPredecessor(Dest, LI, SE, DT)) {
                // Dest has been folded into Fold. Update our worklists
                // accordingly.
    ...
    // At this point, the code is well formed.  We now simplify the unrolled
    // loop, doing constant propagation and dead code elimination as we go.
    simplifyLoopAfterUnroll(L, !CompletelyUnroll && (Count > 1 || Peeled), LI,
                            SE, DT, AC);
    ...
    // After complete unrolling most of the blocks should be contained in
    // OuterL. However, some of them might happen to be out of OuterL (e.g. if
    // they precede a loop exit). In this case we might need to insert PHI nodes
    // in order to preserve LCSSA form. We don't need to check this if we
    // already know that we need to fix LCSSA form.
    // TODO: For now we just recompute LCSSA for the outer loop in this case,
    // but it should be possible to fix it in-place.
    if (PreserveLCSSA && OuterL && CompletelyUnroll && !NeedToFixLCSSA)
        NeedToFixLCSSA |=
            ::needToInsertPhisForLCSSA(OuterL, UnrolledLoopBlocks, LI);

    // If we have a pass and a DominatorTree we should re-simplify impacted
    // loops to ensure subsequent analyses can rely on this form. We want to
    // simplify at least one layer outside of the loop that was unrolled so that
    // any changes to the parent loop exposed by the unrolling are considered.
    if (DT) {
        if (OuterL) {
            // OuterL includes all loops for which we can break loop-simplify,
            // so it's sufficient to simplify only it (it'll recursively
            // simplify inner loops too).
            if (NeedToFixLCSSA) {
                // LCSSA must be performed on the outermost affected loop. The
                // unrolled loop's last loop latch is guaranteed to be in the
                // outermost loop after LoopInfo's been updated by
                // LoopInfo::erase.
            ...
            // TODO: That potentially might be compile-time expensive. We should
            // try to fix the loop-simplified form incrementally.
    ...
    return CompletelyUnroll ? CustomLoopUnrollResult::FullyUnrolled
                            : CustomLoopUnrollResult::PartiallyUnrolled;
}

// @ours
CustomLoopUnrollResult DefaultUnrollLoop(
    Loop *L, unsigned Count, unsigned TripCount, LoopInfo *LI,
    AssumptionCache *AC,
    std::unordered_map<Instruction *, std::vector<Instruction *>> &instHooks) {
    return CustomUnrollLoop(L, Count, TripCount, true, false, true, false,
                            false, 1, 0, true, LI, nullptr, nullptr, AC,
                            nullptr, true, instHooks);
}
